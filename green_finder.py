...
with tab2:  
    ...
    st.plotly_chart(fig)
with tab3:
    st.dataframe(tmp3[['通称線','走行方向','date','集計キロ程','建築限界判定','建築限界判定_側方上部','建築限界判定_側方上部(窓部)','建築限界判定_下部','建築限界判定_側方下部','建築限界判定_上部','車両限界判定','車両限界判定_側方上部','車両限界判定_側方上部(窓部)','車両限界判定_下部','車両限界判定_側方下部','車両限界判定_上部']])
with tab4:
    st.write("""## 🌳 使用手順""")
    st.markdown('''
